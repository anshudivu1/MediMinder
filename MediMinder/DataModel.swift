...
struct User: Codable {
    ...
    // Constructor to create a User from AppUser
    init(from appUser: AppUser) {
        self.name = appUser.name
        self.age = appUser.age ?? 30
        self.gender = appUser.gender ?? "Prefer not to say"
        self.medicalConditions = appUser.medicalConditions
        self.breakfastTime = appUser.breakfastTime ?? Date()
        self.lunchTime = appUser.lunchTime ?? Date()
        self.dinnerTime = appUser.dinnerTime ?? Date()
        self.bedtime = appUser.bedtime ?? Date()
    }
...
extension AppUser {
    func toUser() -> User {
        return User(
            name: self.name,
            age: self.age ?? 30,
            gender: self.gender ?? "Prefer not to say",
            medicalConditions: self.medicalConditions,
            breakfastTime: self.breakfastTime ?? Date(),
            lunchTime: self.lunchTime ?? Date(),
            dinnerTime: self.dinnerTime ?? Date(),
            bedtime: self.bedtime ?? Date()
        )
    }
}
